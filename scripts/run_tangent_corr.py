...
def run_tangent_corr():
    ...
    for attr, value in poly.__dict__.iteritems():
        if attr.startswith('ete_stats'):
            utils.write_json(value.mean.tolist(), "{dir}/{name}_m{m}_t{t}_{s}_pe{p}_{d}_mean.json".format(
                dir=args.od, name=args.o, m=args.mn, t=args.t, s=args.sn, p=args.pe, d=attr))
            utils.write_json(value.variance.tolist(), "{dir}/{name}_m{m}_t{t}_{s}_pe{p}_{d}_var.json".format(
                dir=args.od, name=args.o, m=args.mn, t=args.t, s=args.sn, p=args.pe, d=attr))
            utils.write_json(value.std_error.tolist(), "{dir}/{name}_m{m}_t{t}_{s}_pe{p}_{d}_std_error.json".format(
                dir=args.od, name=args.o, m=args.mn, t=args.t, s=args.sn, p=args.pe, d=attr))
        ...
        if attr.startswith('tangent_corr'):
            ...
            utils.write_json(mean_list, "{dir}/{name}_m{m}_t{t}_{s}_pe{p}_{d}_mean.json".format(
                dir=args.od, name=args.o, m=args.mn, t=args.t, s=args.sn, p=args.pe, d=attr))
            var_list = [i.variance for i in value]
            utils.write_json(var_list, "{dir}/{name}_m{m}_t{t}_{s}_pe{p}_{d}_var.json".format(
                dir=args.od, name=args.o, m=args.mn, t=args.t, s=args.sn, p=args.pe, d=attr))
            std_error_list = [i.std_error for i in value]
            utils.write_json(std_error_list, "{dir}/{name}_m{m}_t{t}_{s}_pe{p}_{d}_std_error.json".format(
                dir=args.od, name=args.o, m=args.mn, t=args.t, s=args.sn, p=args.pe, d=attr))
